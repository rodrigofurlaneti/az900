...
flowchart TD
    ...
    subgraph Fase1 [Parte 1: Consultas Prévias e Preparação]
        ...
        D{"Cruzar Dados e Definir Variáveis"}:::preparacao
    ...
    subgraph Fase2 [Parte 2: Execução da Infraestrutura]
        ...
        G("Passo 4: Criar Máquina Virtual (Sem IP Público)"):::execucao
        H("Passo 5: Criar Regra de Desligamento Automático"):::execucao
...
## 5. Criar a Regra de Desligamento Automático
> **Atenção:** Este comando **falhará** se a sua região (ex: `denmarkeast`) não constar na lista do Passo 3. O Azure exige que o agendamento fique na mesma região da VM. Se a sua região for nova/restrita, ignore este passo.
```bash
az vm auto-shutdown --resource-group $RG --name $VM_NOME --time 1600 --location $REGIAO
```
